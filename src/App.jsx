    ...
    <div style={{background:C.bg,minHeight:"100vh",color:C.text,fontFamily:"-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif"}}>
      ...
      <div style={{maxWidth:600,margin:"0 auto",padding:"12px 16px"}}>
        ...
        {tab==="admin"&&myRole==="admin"&&(<div>
          ...
          <Card style={{borderColor:C.amber+"44"}}><div style={{fontWeight:700,fontSize:15,marginBottom:4,color:C.amber}}>Scores & Games</div><div style={{fontSize:12,color:C.dim,marginBottom:12}}>Refresh data from APIs</div><div style={{display:"flex",flexDirection:"column",gap:8}}>
            <div style={{display:"flex",gap:8}}><Btn small bg={C.amber} onClick={async function(){try{await api("/api/admin/refresh-scores",{method:"POST"});showMsg("Scores refreshed!");await refreshPool();}catch(e){showMsg(e.message,"error");}}}>Refresh Scores</Btn><Btn small bg={C.blue} onClick={async function(){try{var d=await api("/api/admin/refresh-odds",{method:"POST"});showMsg("Odds: "+d.gamesUpdated+" games");setGames(await api("/api/games/upcoming"));}catch(e){showMsg(e.message,"error");}}}>Refresh Odds</Btn></div>
            <div style={{display:"flex",gap:8}}><Btn small bg={C.red} onClick={async function(){try{var d=await api("/api/admin/finalize-games",{method:"POST"});showMsg("Finalized "+d.gamesFinalized+" stuck games");await refreshPool();}catch(e){showMsg(e.message,"error");}}}>Fix Stuck Games</Btn><Btn small bg={C.purple} onClick={async function(){try{var d=await api("/api/admin/import-espn-games",{method:"POST"});showMsg("ESPN: "+d.espnImported+" imported, Odds: "+d.oddsRefreshed+" refreshed");setGames(await api("/api/games/upcoming"));}catch(e){showMsg(e.message,"error");}}}>Import ESPN Games</Btn><Btn small bg="#374151" onClick={async function(){try{var d=await api("/api/admin/dedupe-games",{method:"POST"});showMsg("Removed "+d.duplicatesRemoved+" duplicate games");setGames(await api("/api/games/upcoming"));loadAdminGames();}catch(e){showMsg(e.message,"error");}}}>Remove Duplicates</Btn></div>
