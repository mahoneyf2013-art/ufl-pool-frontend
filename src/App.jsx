...
function TmLogo({team,size}){
  const s=size||20;
  if(team?.logo)return <img src={team.logo} alt={team.abbr} style={{width:s,height:s,objectFit:"contain"}}/>;
  return <div style={{width:s,height:s,borderRadius:"50%",background:team?.color||"#666"}}/>;
}
...
function Card({children,style,onClick}){
  return <div onClick={onClick} style={{background:C.card,borderRadius:12,padding:16,marginBottom:10,border:"1px solid "+C.border,...style}}>{children}</div>;
}

function Btn({children,bg,full,small,disabled,onClick,style}){
  return <button disabled={disabled} onClick={onClick} style={{background:disabled?C.muted:(bg||C.blue),color:"#fff",border:"none",borderRadius:8,padding:small?"6px 14px":"11px 22px",cursor:disabled?"not-allowed":"pointer",fontWeight:700,fontSize:small?12:14,width:full?"100%":"auto",opacity:disabled?0.5:1,...style}}>{children}</button>;
}

function Input(p){
  return <input {...p} style={{background:"#0c0e14",border:"1px solid "+C.border,borderRadius:8,padding:"11px 14px",color:"#fff",fontSize:14,width:"100%",outline:"none",boxSizing:"border-box",...p.style}}/>;
}

function Badge({text,color}){
  return <span style={{background:color+"22",color:color,fontSize:11,fontWeight:700,padding:"2px 8px",borderRadius:20,textTransform:"uppercase"}}>{text}</span>;
}

function Spinner(){
  return <div style={{width:20,height:20,border:"3px solid "+C.blue+"33",borderTop:"3px solid "+C.blue,borderRadius:"50%",animation:"spin 0.8s linear infinite"}}/>;
}

function LoadingCard({text}){
  return <Card style={{textAlign:"center",padding:30,color:C.muted}}><div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:8}}><Spinner/><span>{text||"Loading..."}</span></div></Card>;
}
...
function ConfirmModal({title,message,details,onConfirm,onCancel,confirmText,confirmColor}){
  return (
    <div style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:"rgba(0,0,0,0.7)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:100,padding:20}}>
      <div style={{background:C.card,borderRadius:12,padding:24,maxWidth:400,width:"100%",border:"1px solid "+C.border}}>
        <div style={{fontWeight:700,fontSize:17,marginBottom:8}}>{title}</div>
        <div style={{fontSize:14,color:C.dim,marginBottom:12}}>{message}</div>
        {details && <div style={{background:C.cardB,borderRadius:8,padding:12,marginBottom:16,fontSize:13}}>{details}</div>}
        <div style={{display:"flex",gap:8}}>
          <Btn full bg={confirmColor||C.blue} onClick={onConfirm}>{confirmText||"Confirm"}</Btn>
          <Btn full bg="#374151" onClick={onCancel}>Cancel</Btn>
        </div>
      </div>
    </div>
  );
...
function BetBtn({label,sub,odds,isParlay,onBet,onParlay,disabled,disabledMsg,gameTime}){
  ...
  const profit=calcPayout(parseInt(wager)||0,odds);
  const oddsStr=fmtOdds(odds);
  const locked=gameTime&&(new Date(gameTime).getTime()-300000)<=Date.now();

  if(locked) return (
    <button disabled style={{background:"#1a1f2b",border:"1px solid #252a38",borderRadius:8,padding:"8px 4px",color:"#4b5563",cursor:"not-allowed",textAlign:"center",width:"100%",opacity:0.5}}>
      <div style={{fontWeight:700,fontSize:13}}>{label}</div>
      <div style={{fontSize:9}}>Locked</div>
    </button>
  );

  if(disabled&&isParlay) return (
    <button disabled style={{background:"#1a1f2b",border:"1px solid #252a38",borderRadius:8,padding:"8px 4px",color:"#4b5563",cursor:"not-allowed",textAlign:"center",width:"100%",opacity:0.5}}>
      <div style={{fontWeight:700,fontSize:13}}>{label}</div>
      <div style={{fontSize:9}}>{disabledMsg}</div>
    </button>
  );

  if(isParlay) return (
    <button onClick={onParlay} style={{background:"#1e293b",border:"1px solid #334155",borderRadius:8,padding:"8px 4px",color:"#e5e7eb",cursor:"pointer",textAlign:"center",width:"100%"}} onMouseEnter={function(e){e.currentTarget.style.borderColor="#f59e0b";}} onMouseLeave={function(e){e.currentTarget.style.borderColor="#334155";}}>
      <div style={{fontWeight:700,fontSize:13}}>{label}</div>
      <div style={{fontSize:10,color:C.amber}}>{oddsStr}</div>
    </button>
  );

  if(!open) return (
    <button onClick={function(){setOpen(true);}} style={{background:"#1e293b",border:"1px solid #334155",borderRadius:8,padding:"8px 4px",color:"#e5e7eb",cursor:"pointer",textAlign:"center",width:"100%"}} onMouseEnter={function(e){e.currentTarget.style.borderColor="#3b82f6";}} onMouseLeave={function(e){e.currentTarget.style.borderColor="#334155";}}>
      <div style={{fontWeight:700,fontSize:13}}>{label}</div>
      <div style={{fontSize:10,color:C.dim}}>{sub} • {oddsStr}</div>
    </button>
  );

  return (
    <div style={{background:"#1e293b",border:"1px solid #3b82f6",borderRadius:8,padding:6}}>
      <div style={{fontWeight:700,fontSize:12,marginBottom:2}}>{label}</div>
      <div style={{fontSize:10,color:C.dim,marginBottom:4}}>Odds: {oddsStr}</div>
      <input type="number" placeholder="Wager" value={wager} onChange={function(e){setWager(e.target.value);}} autoFocus onKeyDown={function(e){if(e.key==="Enter"&&wager){onBet(parseInt(wager));setOpen(false);setWager("");}}} style={{background:"#0c0e14",border:"1px solid #2a2d35",borderRadius:6,padding:"5px 8px",color:"#fff",fontSize:12,width:"100%",textAlign:"center",outline:"none",boxSizing:"border-box"}}/>
      {wager&&parseInt(wager)>0 && (
        <div style={{marginTop:4,padding:"4px 6px",background:"#052e16",borderRadius:4,textAlign:"center"}}>
          <span style={{fontSize:11,color:C.green}}>Win: +{profit} → <strong>{parseInt(wager)+profit}</strong></span>
        </div>
      )}
      <div style={{display:"flex",gap:4,marginTop:4}}>
        <button onClick={function(){if(wager){onBet(parseInt(wager));setOpen(false);setWager("");}}} style={{background:"#3b82f6",color:"#fff",border:"none",borderRadius:4,padding:"4px 0",cursor:"pointer",fontWeight:600,fontSize:11,flex:1}}>Place Bet</button>
        <button onClick={function(){setOpen(false);setWager("");}} style={{background:"#374151",color:"#94a3b8",border:"none",borderRadius:4,padding:"4px 6px",cursor:"pointer",fontSize:11}}>✕</button>
      </div>
    </div>
  );
}
...
function LineHistoryPanel({data}){
  ...
  return (
    <div>
      <div style={{display:"grid",gridTemplateColumns:"1.2fr 1fr 1fr 1fr",gap:4,marginBottom:6}}>
        <div style={{fontSize:10,color:C.dim,fontWeight:700}}>Date</div>
        <div style={{fontSize:10,color:C.dim,fontWeight:700}}>Spread</div>
        <div style={{fontSize:10,color:C.dim,fontWeight:700}}>Total</div>
        <div style={{fontSize:10,color:C.dim,fontWeight:700}}>ML (H/A)</div>
      </div>
      {data.history.map(function(h,i){
        const p=i>0?data.history[i-1]:null;
        const sc=p&&p.spread_home!==h.spread_home;
        const tc=p&&p.total!==h.total;
        const mc=p&&p.moneyline_home!==h.moneyline_home;
        const sd=p&&h.spread_home!=null&&p.spread_home!=null?(h.spread_home<p.spread_home?"↓":"↑"):"";
        const td=p&&h.total!=null&&p.total!=null?(h.total>p.total?"↑":"↓"):"";
        return (
          <div key={h.id} style={{display:"grid",gridTemplateColumns:"1.2fr 1fr 1fr 1fr",gap:4,padding:"3px 0",borderBottom:"1px solid "+C.border+"22",fontSize:12}}>
      ...
      {data.current && (
        <div style={{display:"grid",gridTemplateColumns:"1.2fr 1fr 1fr 1fr",gap:4,padding:"4px 0",fontSize:12,borderTop:"1px solid "+C.blue+"44",marginTop:4}}>
...
function AppInner(){
  ...
  var handleAuth=async function(){
    ...
    try{
      if(forgotMode){
        if(!forgotEmail){showMsg("Enter email","error");return;}
        await api("/api/auth/forgot-password",{method:"POST",body:JSON.stringify({email:forgotEmail})});
        showMsg("Check your inbox!");setForgotMode(false);setForgotEmail("");return;
      }
      if(resetToken){
  ...
  var createPool=async function(){
    ...
    try{
      var d=await api("/api/pools",{method:"POST",body:JSON.stringify({name:createForm.name,startingBalance:createForm.balance,requireApproval:createForm.approval})});
  ...
  var joinPool=async function(){
    if(!joinCode)return showMsg("Enter code","error");
    try{
      var d=await api("/api/pools/"+joinCode+"/join",{method:"POST"});
      showMsg(d.status==="pending"?"Request sent!":"Joined!");
      setPools(await api("/api/my-pools"));setJoinCode("");
    }catch(e){showMsg(e.message,"error");}
  };
  ...
  var enterPool=async function(p){
    ...
    try{
      var results=await Promise.all([api("/api/games/upcoming"),api("/api/pools/"+p.id+"/leaderboard")]);
      setGames(results[0]);
      setLeaderboard(results[1].map(function(x){return{...x,isYou:x.username===user.username};}));
      if(p.role==="admin")setMembers(await api("/api/pools/"+p.id+"/members"));
      setBets(await api("/api/pools/"+p.id+"/my-bets"));
      try{setActivity(await api("/api/pools/"+p.id+"/activity"));}catch(e){}
      try{setMessages(await api("/api/pools/"+p.id+"/messages"));}catch(e){}
    }catch(e){showMsg("Failed to load","error");}finally{setLoading(false);}
  };

  var LOCK_MS=300000;
  var isLocked=function(t){return t&&(new Date(t).getTime()-LOCK_MS)<=Date.now();};

  var placeBetDirect=async function(gid,bt,pick,line,odds,wager){
    try{
      var d=await api("/api/bet",{method:"POST",body:JSON.stringify({pool_id:activePool.id,game_id:gid,bet_type:bt,pick:pick,line:line,odds:odds,wager:wager})});
      setBalance(d.newBalance);
      setBets(function(prev){return[{id:"b_"+Date.now(),game_id:gid,bet_type:bt,pick:pick,line:line,odds:odds,wager:wager,result:"pending",potential_profit:calcPayout(wager,odds)},...prev];});
  ...
  var placeBet=async function(gid,bt,pick,line,odds,wager){
    ...
    if(wager>balance*0.25){
      setBetConfirm({gid:gid,bt:bt,pick:pick,line:line,odds:odds,wager:wager,
        pickLabel:bt==="spread"?tm(pick).abbr+" "+(line>0?"+":"")+line:bt==="over"?"Over "+line:bt==="under"?"Under "+line:tm(pick).abbr+" ML ("+fmtOdds(odds)+")",
  ...
  var isParlayConflict=function(gid,bt,pick){
    for(var i=0;i<parlayLegs.length;i++){
      var l=parlayLegs[i];
      if(l.gameId!==gid)continue;
      if((l.betType==="spread"||l.betType==="moneyline")&&(bt==="spread"||bt==="moneyline")&&l.pick===pick)return"Can't parlay spread+ML";
      if(l.betType===bt&&l.pick===pick)return"Already added";
    }
    return null;
  };

  var addParlayLeg=function(gid,bt,pick,line,odds,gl){
    var c=isParlayConflict(gid,bt,pick);
    if(c)return showMsg(c,"error");
    setParlayLegs(function(prev){return[...prev,{gameId:gid,betType:bt,pick:pick,line:line,odds:odds,gameLabel:gl}];});
  };

  var placeParlay=async function(){
    var w=parseInt(parlayWager);
    if(!w||w<=0||parlayLegs.length<2)return showMsg("Need 2+ legs and wager","error");
  ...
  var updateMember=async function(mid,action){
    try{
      await api("/api/pools/"+activePool.id+"/members/"+mid+"/"+action,{method:"POST"});
      setMembers(await api("/api/pools/"+activePool.id+"/members"));
      showMsg("Member "+action+"d!");
    }catch(e){showMsg(e.message,"error");}
  };

  var adjustBalance=async function(){
    if(!adjustAmt)return;
    try{
      await api("/api/pools/"+activePool.id+"/members/"+adjustModal.id+"/adjust-balance",{method:"POST",body:JSON.stringify({amount:parseInt(adjustAmt),reason:adjustReason})});
      setMembers(await api("/api/pools/"+activePool.id+"/members"));
      setAdjustModal(null);setAdjustAmt("");setAdjustReason("");showMsg("Adjusted!");
    }catch(e){showMsg(e.message,"error");}
  };
  ...
  var sendMessage=async function(){
    ...
    try{
      var m=await api("/api/pools/"+activePool.id+"/messages",{method:"POST",body:JSON.stringify({content:msgInput})});
      setMessages(function(prev){return[...prev,m];});setMsgInput("");
    }catch(e){showMsg(e.message,"error");}
  };

  var loadLineHistory=async function(gameId,target){
    if(target==="board"){
      if(expandedGame===gameId){setExpandedGame(null);setLineHistory(null);return;}
      setExpandedGame(gameId);
      try{setLineHistory(await api("/api/games/"+gameId+"/line-history"));}catch(e){}
    }else{
      if(schedExpanded===gameId){setSchedExpanded(null);setSchedLineHist(null);setGameSummary(null);return;}
      setSchedExpanded(gameId);setGameSummary(null);
      try{setSchedLineHist(await api("/api/games/"+gameId+"/line-history"));}catch(e){}
    }
  };
  ...
  var viewPlayer=async function(username){
    setViewingPlayer(username);setViewingPlayerData(null);
    try{
      var d=await api("/api/pools/"+activePool.id+"/profile/"+username);
      setViewingPlayerData(d);
    }catch(e){showMsg("Couldn't load profile","error");setViewingPlayer(null);}
  };

  var refreshPool=async function(){
    if(!activePool)return;
    try{
      var results=await Promise.all([api("/api/games/upcoming"),api("/api/pools/"+activePool.id+"/leaderboard"),api("/api/pools/"+activePool.id+"/my-bets")]);
      setGames(results[0]);
      setLeaderboard(results[1].map(function(x){return{...x,isYou:x.username===user.username};}));
      setBets(results[2]);
      ...
      <div style={{background:C.bg,minHeight:"100vh",color:C.text,fontFamily:"-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif"}}>
        <div style={{maxWidth:400,margin:"0 auto",padding:"60px 20px"}}>
          <div style={{textAlign:"center",marginBottom:40}}>
            <div style={{fontSize:44,fontWeight:900,letterSpacing:2,background:"linear-gradient(135deg,#3b82f6,#8b5cf6)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>UFL POOL</div>
            <div style={{color:C.dim,fontSize:14,marginTop:4}}>Fantasy Sportsbook for Your Crew</div>
          </div>
          ...
          {resetToken ? (
            <Card>
              <div style={{textAlign:"center",marginBottom:16}}><div style={{fontSize:18,fontWeight:700}}>Set New Password</div></div>
              <div style={{marginBottom:12}}><label style={{fontSize:12,color:C.dim,display:"block",marginBottom:4}}>New Password</label><Input type="password" value={resetForm.password} onChange={function(e){setResetForm({...resetForm,password:e.target.value});}}/></div>
              <div style={{marginBottom:20}}><label style={{fontSize:12,color:C.dim,display:"block",marginBottom:4}}>Confirm</label><Input type="password" value={resetForm.confirm} onChange={function(e){setResetForm({...resetForm,confirm:e.target.value});}} onKeyDown={function(e){if(e.key==="Enter")handleAuth();}}/></div>
              <Btn full onClick={handleAuth} disabled={authLoading}>{authLoading?"...":"Reset Password"}</Btn>
            </Card>
          ) : forgotMode ? (
            <Card>
              <div style={{textAlign:"center",marginBottom:16}}><div style={{fontSize:18,fontWeight:700}}>Forgot Password</div></div>
              <div style={{marginBottom:20}}><Input type="email" value={forgotEmail} onChange={function(e){setForgotEmail(e.target.value);}} placeholder="your@email.com" onKeyDown={function(e){if(e.key==="Enter")handleAuth();}}/></div>
              <Btn full onClick={handleAuth} disabled={authLoading}>{authLoading?"...":"Send Reset Link"}</Btn>
              <div style={{textAlign:"center",marginTop:12}}><button onClick={function(){setForgotMode(false);}} style={{background:"none",border:"none",color:C.blue,cursor:"pointer",fontSize:13}}>← Back</button></div>
            </Card>
          ) : (
            <Card>
              <div style={{display:"flex",marginBottom:20,borderRadius:8,overflow:"hidden",border:"1px solid "+C.border}}>
                {["login","register"].map(function(v){return <button key={v} onClick={function(){setAuthView(v);}} style={{flex:1,padding:"10px 0",background:authView===v?C.blue:"transparent",color:"#fff",border:"none",cursor:"pointer",fontWeight:700,fontSize:14}}>{v==="login"?"Sign In":"Sign Up"}</button>;})}
              </div>
  ...
  if(screen==="pools"){
    return (
      <div style={{background:C.bg,minHeight:"100vh",color:C.text,fontFamily:"-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif"}}>
        <div style={{background:"linear-gradient(135deg,#1e3a5f,#0f172a)",padding:"16px 20px",borderBottom:"2px solid "+C.blue}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",maxWidth:600,margin:"0 auto"}}>
            <div><div style={{fontSize:22,fontWeight:800,letterSpacing:1}}>UFL POOL</div><div style={{fontSize:12,color:C.dim}}>Welcome, {user.displayName}</div></div>
            <Btn small bg="#374151" onClick={logout}>Sign Out</Btn>
          </div>
        </div>
        <div style={{maxWidth:600,margin:"0 auto",padding:"16px 20px"}}>
          ...
          {pools.map(function(p){
            return (
              <Card key={p.id} onClick={function(){enterPool(p);}} style={{cursor:p.status==="pending"?"default":"pointer",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                <div>
                  <div style={{fontWeight:700,fontSize:15,display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
                    {p.name}
                    <Badge text={p.role} color={p.role==="admin"?C.purple:C.blue}/>
                    {p.status==="pending" && <Badge text="Pending" color={C.amber}/>}
                  </div>
                  <div style={{fontSize:12,color:C.dim,marginTop:3}}>Code: {p.join_code} • {p.member_count||1} members</div>
                </div>
                {p.status==="active" && <div style={{fontWeight:800,fontSize:18,color:(p.balance||1000)>=1000?C.green:C.red}}>{(p.balance||1000).toLocaleString()}</div>}
              </Card>
            );
          })}
          ...
          <Card>
            <Input placeholder="Pool Name" value={createForm.name} onChange={function(e){setCreateForm({...createForm,name:e.target.value});}} style={{marginBottom:10}}/>
            <div style={{display:"flex",gap:10,marginBottom:12}}>
              <div style={{flex:1}}><label style={{fontSize:12,color:C.dim}}>Starting Balance</label><Input type="number" value={createForm.balance} onChange={function(e){setCreateForm({...createForm,balance:parseInt(e.target.value)||1000});}}/></div>
              <div style={{flex:1}}><label style={{fontSize:12,color:C.dim}}>Require Approval</label><div style={{marginTop:6}}><button onClick={function(){setCreateForm({...createForm,approval:!createForm.approval});}} style={{background:createForm.approval?C.green+"33":C.muted+"33",border:"1px solid "+(createForm.approval?C.green:C.muted),borderRadius:20,padding:"6px 16px",color:createForm.approval?C.green:C.muted,cursor:"pointer",fontWeight:600,fontSize:13}}>{createForm.approval?"Yes":"No"}</button></div></div>
            </div>
            <Btn full onClick={createPool}>Create Pool</Btn>
          </Card>

          <div style={{fontSize:15,fontWeight:700,margin:"24px 0 10px"}}>Join a Pool</div>
          <Card>
            <div style={{display:"flex",gap:8}}>
              <Input placeholder="Join code" value={joinCode} onChange={function(e){setJoinCode(e.target.value.toUpperCase());}} onKeyDown={function(e){if(e.key==="Enter")joinPool();}} style={{flex:1}}/>
              <Btn bg={C.purple} onClick={joinPool}>Join</Btn>
            </div>
          </Card>
        </div>
      </div>
  ...
  var switchSection=function(sid){
    setSection(sid);
    var nav=navItems.find(function(n){return n.id===sid;});
    ...
    <div style={{background:C.bg,minHeight:"100vh",color:C.text,fontFamily:"-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif"}}>
      ...
      <div style={{position:"fixed",top:0,left:sidebarOpen?0:-280,width:270,height:"100vh",background:"#0f1318",borderRight:"1px solid "+C.border,zIndex:300,transition:"left 0.25s ease",display:"flex",flexDirection:"column",overflow:"hidden"}}>
        <div style={{padding:"20px 16px",borderBottom:"1px solid "+C.border}}>
          <div style={{fontSize:22,fontWeight:900,letterSpacing:1,background:"linear-gradient(135deg,#3b82f6,#8b5cf6)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>UFL POOL</div>
          <div style={{fontSize:12,color:C.dim,marginTop:4}}>{activePool?.name}</div>
        </div>
        <div style={{flex:1,overflowY:"auto",padding:"8px 0"}}>
          {navItems.map(function(nav){
            return (
              <button key={nav.id} onClick={function(){switchSection(nav.id);}} style={{display:"flex",alignItems:"center",gap:12,width:"100%",padding:"14px 20px",background:section===nav.id?C.blue+"22":"transparent",border:"none",borderLeft:section===nav.id?"3px solid "+C.blue:"3px solid transparent",color:section===nav.id?C.text:C.dim,cursor:"pointer",fontSize:15,fontWeight:section===nav.id?700:500,textAlign:"left"}}>
                <span style={{fontSize:20}}>{nav.icon}</span><span>{nav.label}</span>
              </button>
            );
          })}
        </div>
        <div style={{padding:"12px 16px",borderTop:"1px solid "+C.border}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
            <div><div style={{fontSize:13,fontWeight:600}}>{user.displayName}</div><div style={{fontSize:11,color:C.dim}}>@{user.username}</div></div>
            <div style={{fontWeight:800,fontSize:16,color:balance>=1000?C.green:C.red}}>{balance.toLocaleString()}</div>
          </div>
          <button onClick={function(){setSidebarOpen(false);setLeaveConfirm(true);}} style={{width:"100%",padding:"8px 0",background:"#1e293b",border:"1px solid "+C.border,borderRadius:8,color:C.dim,cursor:"pointer",fontSize:12,fontWeight:600}}>← Back to Pools</button>
        </div>
      </div>
      ...
      <div style={{background:"linear-gradient(135deg,#1e3a5f,#0f172a)",padding:"12px 16px",borderBottom:"2px solid "+C.blue,position:"sticky",top:0,zIndex:100}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",maxWidth:600,margin:"0 auto"}}>
          <div style={{display:"flex",alignItems:"center",gap:12}}>
            <button onClick={function(){setSidebarOpen(true);}} style={{background:"none",border:"none",color:C.text,cursor:"pointer",fontSize:22,padding:0,lineHeight:1}}>☰</button>
            <div><div style={{fontSize:16,fontWeight:800}}>{activePool?.name}</div><div style={{fontSize:11,color:C.dim}}>{currentNav?.icon} {currentNav?.label}</div></div>
          </div>
          <div style={{textAlign:"right"}}>
            <div style={{fontSize:22,fontWeight:800,color:balance>=1000?C.green:balance>=500?C.amber:C.red}}>{balance.toLocaleString()}</div>
      ...
      <div style={{maxWidth:600,margin:"0 auto",padding:"12px 16px"}}>
        {/* Sub-tabs */}
        {currentNav?.subs?.length>1 && (
          <div style={{display:"flex",gap:4,marginBottom:14}}>
            {currentNav.subs.map(function(s){
              return <button key={s[0]} onClick={function(){setSubTab(s[0]);}} style={{background:subTab===s[0]?C.blue:C.card,color:"#fff",border:subTab===s[0]?"none":"1px solid "+C.border,borderRadius:8,flex:1,padding:"9px 4px",cursor:"pointer",fontWeight:600,fontSize:13}}>{s[1]}</button>;
            })}
          </div>
        )}
        ...
        {betConfirm && (
          <ConfirmModal
            title="Confirm Large Wager"
            message={"This is "+Math.round(betConfirm.wager/balance*100)+"% of your balance."}
            details={
              <div>
                <div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}><span style={{color:C.dim}}>Pick:</span><strong>{betConfirm.pickLabel}</strong></div>
                <div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}><span style={{color:C.dim}}>Wager:</span><strong>{betConfirm.wager} pts</strong></div>
                <div style={{display:"flex",justifyContent:"space-between"}}><span style={{color:C.dim}}>To win:</span><strong style={{color:C.green}}>+{betConfirm.profit}</strong></div>
              </div>
            }
            onConfirm={async function(){var bc={...betConfirm};setBetConfirm(null);await placeBetDirect(bc.gid,bc.bt,bc.pick,bc.line,bc.odds,bc.wager);}}
            onCancel={function(){setBetConfirm(null);}}
        ...
        {(tab==="board"||tab==="parlay") && (
          <div>
            ...
            {games.filter(function(g){return g.status==="upcoming";}).map(function(game){
              var aw=tm(game.away_team),hm=tm(game.home_team),isP=tab==="parlay";
              var time=new Date(game.commence_time),locked=isLocked(game.commence_time);
              var cSA=isP?isParlayConflict(game.id,"spread",game.away_team):null;
              var cSH=isP?isParlayConflict(game.id,"spread",game.home_team):null;
              var cMA=isP?isParlayConflict(game.id,"moneyline",game.away_team):null;
              var cMH=isP?isParlayConflict(game.id,"moneyline",game.home_team):null;
              return (
                <Card key={game.id} style={locked?{opacity:0.5}:{}}>
                  <div onClick={function(){loadLineHistory(game.id,"board");}} style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10,cursor:"pointer"}}>
                    <div style={{display:"flex",alignItems:"center",gap:8}}>
                      <TmLogo team={aw} size={24}/><span style={{fontWeight:700,fontSize:15}}>{aw.abbr}</span>
                      <span style={{color:C.muted}}>@</span>
                      <span style={{fontWeight:700,fontSize:15}}>{hm.abbr}</span><TmLogo team={hm} size={24}/>
                    </div>
                    <div style={{textAlign:"right"}}>
                      <div style={{fontSize:11,color:locked?C.red:C.muted}}>{locked?"LOCKED":time.toLocaleDateString("en-US",{weekday:"short"})+" "+time.toLocaleTimeString("en-US",{hour:"numeric",minute:"2-digit"})}</div>
                      <div style={{fontSize:10,color:C.blue}}>{expandedGame===game.id?"▲ Hide":"▼ Lines"}</div>
                    </div>
                  </div>
                  {expandedGame===game.id && lineHistory && (
                    <div style={{marginBottom:10,padding:10,background:C.cardB,borderRadius:8}}>
                      <div style={{fontWeight:700,fontSize:13,marginBottom:8,color:C.amber}}>Line Movement</div>
                      <LineHistoryPanel data={lineHistory}/>
                    </div>
                  )}
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:5,marginBottom:4}}>
                    <div style={{fontSize:10,color:C.muted,textAlign:"center",fontWeight:600,textTransform:"uppercase",letterSpacing:1}}>Spread</div>
                    <div style={{fontSize:10,color:C.muted,textAlign:"center",fontWeight:600,textTransform:"uppercase",letterSpacing:1}}>Total</div>
                    <div style={{fontSize:10,color:C.muted,textAlign:"center",fontWeight:600,textTransform:"uppercase",letterSpacing:1}}>ML</div>
                  </div>
        ...
        {/* PARLAY SLIP */}
        {tab==="parlay" && parlayLegs.length>0 && (
          <Card style={{border:"1px solid "+C.amber,position:"sticky",bottom:10,zIndex:10}}>
            <div style={{fontWeight:700,marginBottom:8,color:C.amber,fontSize:15}}>Parlay — {parlayLegs.length} Legs</div>
            {parlayLegs.map(function(l,i){
              var t=tm(l.pick);
              var d=l.betType==="spread"?t.abbr+" "+(l.line>0?"+":"")+l.line:l.betType==="over"?"Over "+l.line:l.betType==="under"?"Under "+l.line:t.abbr+" ML ("+fmtOdds(l.odds)+")";
              return (
                <div key={i} style={{display:"flex",justifyContent:"space-between",padding:"4px 0",fontSize:13,borderBottom:i<parlayLegs.length-1?"1px solid "+C.border:"none"}}>
                  <span>{l.gameLabel}: <strong>{d}</strong></span>
                  <button onClick={function(){setParlayLegs(parlayLegs.filter(function(_,j){return j!==i;}));}} style={{background:"none",border:"none",color:C.red,cursor:"pointer",fontWeight:700}}>×</button>
                </div>
              );
            })}
            <div style={{display:"flex",gap:8,marginTop:10}}>
              <Input type="number" placeholder="Wager" value={parlayWager} onChange={function(e){setParlayWager(e.target.value);}} style={{flex:1}}/>
              <Btn bg={C.amber} onClick={placeParlay}>Place</Btn>
            </div>
            {parlayWager && parseInt(parlayWager)>0 && (
              <div style={{marginTop:8,padding:"6px 10px",background:"#052e16",borderRadius:6,textAlign:"center"}}>
                <span style={{fontSize:13,color:C.green,fontWeight:600}}>Win: +{calcParlayPayout(parlayLegs,parseInt(parlayWager))}</span>
              </div>
            )}
          </Card>
        )}
            ...
            {schedule.length>0 && (
              <div>
                <div style={{display:"flex",gap:4,marginBottom:12,overflowX:"auto",paddingBottom:4}}>
                  {schedule.map(function(w){
                    return <button key={w.week} onClick={function(){setScheduleWeek(w.week);setSchedExpanded(null);setSchedLineHist(null);setGameSummary(null);}} style={{background:scheduleWeek===w.week?C.blue:C.card,color:"#fff",border:scheduleWeek===w.week?"none":"1px solid "+C.border,borderRadius:8,padding:"6px 12px",cursor:"pointer",fontWeight:600,fontSize:12,whiteSpace:"nowrap"}}>Wk {w.week}</button>;
                  })}
                </div>
                {schedule.filter(function(w){return w.week===scheduleWeek;}).map(function(wk){
                  return wk.games.map(function(g,gi){
                    ...
                    var fin=g.status==="final"||g.status==="STATUS_FINAL";
                    var time=new Date(g.date||g.commence_time);
                    var gk=g.odds_game_id||g.id||gi;
                    var isExp=schedExpanded===gk;
                    return (
                      <Card key={gk}>
                        <div onClick={function(){
                          ...
                          {fin ? (
                            <div style={{display:"flex",justifyContent:"center",gap:20,marginBottom:8,padding:"8px 0",background:"#111",borderRadius:6}}>
                              <div style={{textAlign:"center"}}>
                                <TmLogo team={aw} size={28}/>
                                <div style={{fontSize:12,color:C.dim,marginTop:2}}>{g.away_abbr||aw.abbr}</div>
                                {g.away_record && <div style={{fontSize:10,color:C.muted}}>({g.away_record})</div>}
                                <div style={{fontSize:22,fontWeight:800,color:g.away_score>g.home_score?C.green:C.text}}>{g.away_score}</div>
                              </div>
                              <div style={{alignSelf:"center"}}><Badge text="Final" color={C.muted}/></div>
                              <div style={{textAlign:"center"}}>
                                <TmLogo team={hm} size={28}/>
                                <div style={{fontSize:12,color:C.dim,marginTop:2}}>{g.home_abbr||hm.abbr}</div>
                                {g.home_record && <div style={{fontSize:10,color:C.muted}}>({g.home_record})</div>}
                                <div style={{fontSize:22,fontWeight:800,color:g.home_score>g.away_score?C.green:C.text}}>{g.home_score}</div>
                              </div>
                            </div>
                          ) : (
                            <div style={{display:"flex",justifyContent:"center",gap:16,marginBottom:8,padding:"8px 0",background:"#111",borderRadius:6}}>
                              <div style={{textAlign:"center"}}>
                                <TmLogo team={aw} size={28}/>
                                <div style={{fontWeight:700,fontSize:15,marginTop:4}}>{g.away_abbr||aw.abbr}</div>
                                {g.away_record && <div style={{fontSize:10,color:C.muted}}>({g.away_record})</div>}
                              </div>
                              <div style={{alignSelf:"center",fontSize:12,color:C.dim}}>vs</div>
                              <div style={{textAlign:"center"}}>
                                <TmLogo team={hm} size={28}/>
                                <div style={{fontWeight:700,fontSize:15,marginTop:4}}>{g.home_abbr||hm.abbr}</div>
                                {g.home_record && <div style={{fontSize:10,color:C.muted}}>({g.home_record})</div>}
                              </div>
                            </div>
                          )}
                          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                            <div style={{fontSize:12,color:C.dim}}>
                              ...
                              {g.tv && <span style={{color:C.amber}}> • 📺 {g.tv}</span>}
                              {g.venue && <span> • {g.venue}</span>}
                            </div>
                            <div style={{fontSize:10,color:C.blue}}>{isExp?"▲ Hide":"▼ Details"}</div>
                          </div>
                          {!isExp && g.spread_home!=null && (
                            <div style={{display:"flex",gap:12,marginTop:6,fontSize:12,color:C.dim}}>
                              {g.spread_home!=null && <span>Spd: {g.spread_home>0?"+":""}{g.spread_home}</span>}
                              {g.total!=null && <span>O/U: {g.total}</span>}
                              {g.moneyline_home!=null && <span>ML: {g.moneyline_home}/{g.moneyline_away}</span>}
                            </div>
                          )}
                        </div>
                          ...
                          <div style={{marginTop:10}}>
                            {g.spread_home!=null && (
                              <div style={{padding:10,background:C.cardB,borderRadius:8,marginBottom:8}}>
                                <div style={{fontWeight:700,fontSize:13,marginBottom:6}}>{fin?"Closing":"Current"} Lines</div>
                                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,fontSize:13}}>
                                  <div><div style={{fontSize:10,color:C.dim}}>Spread</div><div style={{fontWeight:600}}>{hm.abbr} {g.spread_home>0?"+":""}{g.spread_home}</div></div>
                                  <div><div style={{fontSize:10,color:C.dim}}>Total</div><div style={{fontWeight:600}}>{g.total||"—"}</div></div>
                                  <div><div style={{fontSize:10,color:C.dim}}>ML</div><div style={{fontWeight:600}}>{g.moneyline_home||"—"}/{g.moneyline_away||"—"}</div></div>
                                </div>
                              </div>
                            )}
                            {schedLineHist && schedExpanded===gk && (
                              <div style={{padding:10,background:C.cardB,borderRadius:8,marginBottom:8}}>
                                <div style={{fontWeight:700,fontSize:13,marginBottom:8,color:C.amber}}>Line Movement</div>
                                <LineHistoryPanel data={schedLineHist}/>
                              </div>
                            )}
          ...
          <div>
            {schedule.length===0 && (
              <div style={{marginBottom:12}}>
                <Btn full onClick={function(){api("/api/schedule").then(function(s){setSchedule(s);if(s.length)setRecapWeek(s[s.length-1].week);}).catch(function(){});}}>Load Weeks</Btn>
              </div>
            )}
            {schedule.length>0 && (
              <div style={{display:"flex",gap:4,marginBottom:12,overflowX:"auto"}}>
                {schedule.map(function(w){
                  return <button key={w.week} onClick={function(){setRecapWeek(w.week);}} style={{background:recapWeek===w.week?C.blue:C.card,color:"#fff",border:recapWeek===w.week?"none":"1px solid "+C.border,borderRadius:8,padding:"6px 12px",cursor:"pointer",fontWeight:600,fontSize:12,whiteSpace:"nowrap"}}>Wk {w.week}</button>;
                })}
              </div>
            )}
            ...
            {recapData && (
              <div>
                <Card style={{borderColor:C.amber+"44"}}>
                  <div style={{fontWeight:800,fontSize:18,color:C.amber,marginBottom:12}}>Week {recapData.week} Recap</div>
                  <div style={{fontSize:12,fontWeight:700,color:C.dim,marginBottom:6,textTransform:"uppercase"}}>Results</div>
                  {recapData.games?.map(function(g,i){
                    var aw=tm(g.away_team),hm=tm(g.home_team);
                    return (
                      <div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"6px 0",borderBottom:"1px solid "+C.border+"22"}}>
                        <div style={{display:"flex",alignItems:"center",gap:6}}><TmLogo team={aw} size={18}/><span style={{fontWeight:g.away_score>g.home_score?700:400,color:g.away_score>g.home_score?C.green:C.text}}>{aw.abbr} {g.away_score}</span></div>
                        <span style={{color:C.muted,fontSize:11}}>vs</span>
                        <div style={{display:"flex",alignItems:"center",gap:6}}><span style={{fontWeight:g.home_score>g.away_score?700:400,color:g.home_score>g.away_score?C.green:C.text}}>{hm.abbr} {g.home_score}</span><TmLogo team={hm} size={18}/></div>
                      </div>
                    );
                  })}
                </Card>
                {recapData.playerSummaries?.length>0 && (
                  <Card>
                    <div style={{fontWeight:700,fontSize:15,marginBottom:10}}>Player Performance</div>
                    {recapData.playerSummaries.map(function(p,i){
                      var pr=parseInt(p.week_profit);
                      return (
                        <div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 0",borderBottom:i<recapData.playerSummaries.length-1?"1px solid "+C.border:"none"}}>
                          <div><span style={{fontWeight:700}}>{i===0?"🏆 ":""}{p.display_name}</span><div style={{fontSize:11,color:C.dim}}>{p.wins}W-{p.losses}L</div></div>
                          <div style={{textAlign:"right"}}><div style={{fontWeight:700,color:pr>=0?C.green:C.red}}>{pr>=0?"+":""}{pr} pts</div></div>
                        </div>
                      );
                    })}
                  </Card>
                ...
                <Card>
                  <div style={{fontWeight:700,fontSize:15,marginBottom:10}}>Highlights</div>
                  {recapData.bigWin && (
                    <div style={{padding:10,background:"#052e1633",borderRadius:8,marginBottom:8}}>
                      <div style={{fontSize:12,color:C.green,fontWeight:700,marginBottom:4}}>💰 Biggest Win</div>
                      <div style={{fontSize:14}}><strong>{recapData.bigWin.display_name}</strong> won <strong style={{color:C.green}}>+{recapData.bigWin.payout-recapData.bigWin.wager}</strong> on {tm(recapData.bigWin.pick).abbr} ({recapData.bigWin.wager} wager)</div>
                    </div>
                  )}
                  {recapData.bigLoss && (
                    <div style={{padding:10,background:"#2c0b0e33",borderRadius:8}}>
                      <div style={{fontSize:12,color:C.red,fontWeight:700,marginBottom:4}}>😤 Biggest Loss</div>
                      <div style={{fontSize:14}}><strong>{recapData.bigLoss.display_name}</strong> lost <strong style={{color:C.red}}>{recapData.bigLoss.wager}</strong> on {tm(recapData.bigLoss.pick).abbr}</div>
                    </div>
                  )}
                  {!recapData.bigWin && !recapData.bigLoss && <div style={{color:C.muted,textAlign:"center",padding:10}}>No graded bets yet</div>}
                </Card>
              </div>
            )}
          </div>
        ...
        {tab==="chat" && (
          <Card style={{padding:0,display:"flex",flexDirection:"column",height:500}}>
            <div style={{padding:"12px 16px",borderBottom:"1px solid "+C.border,fontWeight:700}}>League Chat</div>
            <div style={{flex:1,overflowY:"auto",padding:"8px 16px"}}>
              {messages.length===0 && <div style={{textAlign:"center",color:C.muted,padding:40,fontSize:13}}>Start the trash talk!</div>}
              {messages.map(function(m,i){
                var me=m.username===user.username;
                var sys=m.content.startsWith("[ADMIN]");
                return (
                  <div key={m.id||i} style={{marginBottom:8,textAlign:me?"right":"left"}}>
                    {sys ? (
                      <div style={{background:C.purple+"22",border:"1px solid "+C.purple+"33",borderRadius:8,padding:"6px 10px",fontSize:12,color:C.purple,textAlign:"center"}}>{m.content}</div>
                    ) : (
                      <div>
                        <div style={{fontSize:11,color:C.dim,marginBottom:2}}>{me?"You":m.display_name}</div>
                        <div style={{display:"inline-block",background:me?C.blue+"33":"#1e293b",borderRadius:12,padding:"8px 12px",fontSize:14,maxWidth:"80%",textAlign:"left"}}>{m.content}</div>
                        <div style={{fontSize:10,color:C.muted,marginTop:2}}>{new Date(m.created_at).toLocaleTimeString("en-US",{hour:"numeric",minute:"2-digit"})}</div>
                      </div>
                    )}
                  </div>
                );
            ...
            <div style={{display:"flex",gap:8,padding:"8px 12px",borderTop:"1px solid "+C.border}}>
              <Input placeholder="Type a message..." value={msgInput} onChange={function(e){setMsgInput(e.target.value);}} onKeyDown={function(e){if(e.key==="Enter")sendMessage();}} style={{flex:1}}/>
              <Btn small onClick={sendMessage}>Send</Btn>
            </div>
          </Card>
        )}
        ...
        {tab==="news" && (
          <div>
            <div style={{fontSize:17,fontWeight:700,marginBottom:12}}>UFL News</div>
            {news.length===0 && <LoadingCard text="Loading news..."/>}
            {news.map(function(a,i){
              return (
                <Card key={i} onClick={function(){if(a.links?.web?.href)window.open(a.links.web.href,"_blank");}} style={{cursor:"pointer"}}>
                  {a.images?.[0] && <img src={a.images[0].url} alt="" style={{width:"100%",height:160,objectFit:"cover",borderRadius:8,marginBottom:10}}/>}
                  <div style={{fontWeight:700,fontSize:15,marginBottom:4}}>{a.headline}</div>
                  <div style={{fontSize:13,color:C.dim,lineHeight:1.5,marginBottom:6}}>{a.description?.substring(0,200)}{a.description?.length>200?"...":""}</div>
                  <div style={{display:"flex",justifyContent:"space-between",fontSize:11,color:C.muted}}><span>{a.byline||"ESPN"}</span><span>{new Date(a.published).toLocaleDateString("en-US",{month:"short",day:"numeric"})}</span></div>
                </Card>
              );
            })}
          </div>
        ...
        {tab==="profile" && (
          <div>
            {!profile && <LoadingCard text="Loading profile..."/>}
            {profile && (
              <div>
                <Card style={{textAlign:"center",padding:20}}>
                  <div style={{fontSize:20,fontWeight:800,marginBottom:4}}>{profile.display_name}</div>
                  <div style={{fontSize:13,color:C.dim}}>@{profile.username}</div>
                  <div style={{fontSize:32,fontWeight:800,color:profile.balance>=1000?C.green:C.red,marginTop:8}}>{profile.balance?.toLocaleString()} pts</div>
                  {profile.streak?.count>0 && <div style={{marginTop:6}}><Badge text={profile.streak.count+" "+profile.streak.type+" streak"} color={profile.streak.type==="win"?C.green:C.red}/></div>}
                </Card>
                <Card>
                  <div style={{fontWeight:700,fontSize:15,marginBottom:12}}>Stats</div>
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8}}>
                    <div style={{textAlign:"center",padding:10,background:C.cardB,borderRadius:8}}><div style={{fontSize:20,fontWeight:800,color:C.green}}>{profile.stats?.win_rate||0}%</div><div style={{fontSize:10,color:C.dim}}>Win Rate</div></div>
                    <div style={{textAlign:"center",padding:10,background:C.cardB,borderRadius:8}}><div style={{fontSize:20,fontWeight:800,color:parseFloat(profile.stats?.roi||0)>=0?C.green:C.red}}>{profile.stats?.roi||0}%</div><div style={{fontSize:10,color:C.dim}}>ROI</div></div>
                    <div style={{textAlign:"center",padding:10,background:C.cardB,borderRadius:8}}><div style={{fontSize:20,fontWeight:800}}>{parseInt(profile.stats?.total_bets||0)}</div><div style={{fontSize:10,color:C.dim}}>Bets</div></div>
                  </div>
                </Card>
                <Card>
                  <div style={{fontWeight:700,fontSize:15,marginBottom:8}}>P&L</div>
                  <div style={{display:"flex",justifyContent:"space-between",padding:"6px 0",borderBottom:"1px solid "+C.border}}><span style={{color:C.dim,fontSize:13}}>Total Wagered</span><span style={{fontWeight:600}}>{parseInt(profile.stats?.total_wagered||0).toLocaleString()}</span></div>
                  <div style={{display:"flex",justifyContent:"space-between",padding:"6px 0",borderBottom:"1px solid "+C.border}}><span style={{color:C.dim,fontSize:13}}>Net Profit</span><span style={{fontWeight:700,color:parseInt(profile.stats?.net_profit||0)>=0?C.green:C.red}}>{parseInt(profile.stats?.net_profit||0)>=0?"+":""}{parseInt(profile.stats?.net_profit||0).toLocaleString()}</span></div>
                </Card>
                {profile.byType?.length>0 && (
                  <Card>
                    <div style={{fontWeight:700,fontSize:15,marginBottom:8}}>By Type</div>
                    {profile.byType.map(function(bt,i){
                      var wr=parseInt(bt.total)>0?Math.round((parseInt(bt.wins)/(parseInt(bt.wins)+parseInt(bt.losses)))*100):0;
                      return (
                        <div key={i} style={{display:"flex",justifyContent:"space-between",padding:"6px 0",borderBottom:i<profile.byType.length-1?"1px solid "+C.border:"none"}}>
                          <span style={{fontWeight:600,textTransform:"capitalize"}}>{bt.bet_type}</span>
                          <div style={{display:"flex",gap:12,fontSize:13}}><span style={{color:C.green}}>{bt.wins}W</span><span style={{color:C.red}}>{bt.losses}L</span><span style={{color:C.dim}}>{wr}%</span></div>
                        </div>
                      );
                    })}
                  </Card>
        ...
        {tab==="bets" && (
          <div>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
              <div style={{fontWeight:700,fontSize:17}}>My Bets</div>
        ...
        {tab==="leaderboard" && (
          <Card>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
              <div style={{fontWeight:700,fontSize:17}}>Standings</div>
              <Btn small bg="#374151" onClick={function(){window.open(API_URL+"/api/pools/"+activePool.id+"/export/standings","_blank");}}>CSV</Btn>
            </div>
            {[...leaderboard].sort(function(a,b){return(parseInt(b.display_balance)||b.balance)-(parseInt(a.display_balance)||a.balance);}).map(function(p,i){
              var bal=parseInt(p.display_balance||p.balance);
              var pend=parseInt(p.pending_amount||0);
              return (
                <div key={i} onClick={function(){viewPlayer(p.username);}} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"11px 14px",borderRadius:10,marginBottom:4,background:i===0?"linear-gradient(135deg,#1e3a5f,#0f172a)":p.isYou?"#1a2332":"transparent",border:p.isYou?"1px solid "+C.blue:"1px solid transparent",cursor:"pointer"}}>
                  <div style={{display:"flex",alignItems:"center",gap:12}}>
                    <span style={{fontWeight:900,fontSize:20,color:i===0?"#fbbf24":i===1?"#94a3b8":i===2?"#cd7f32":"#4b5563",width:28}}>{i+1}</span>
                    <div><span style={{fontWeight:600}}>{p.display_name}{p.isYou?" (You)":""}</span><div style={{fontSize:11,color:C.muted}}>{p.wins||0}W-{p.losses||0}L{p.pushes?"-"+p.pushes+"P":""} • <span style={{color:C.blue}}>View</span></div></div>
                  </div>
                  <div style={{textAlign:"right"}}>
                    <span style={{fontWeight:700,fontSize:18,color:bal>=1000?C.green:bal>=500?C.amber:C.red}}>{bal.toLocaleString()}</span>
                    {pend>0 && <div style={{fontSize:11,color:C.amber}}>{pend.toLocaleString()} at risk</div>}
                  </div>
                </div>
              );
            })}
        ...
        {viewingPlayer && (
          <div style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:"rgba(0,0,0,0.75)",zIndex:200,display:"flex",alignItems:"flex-start",justifyContent:"center",padding:"40px 16px",overflowY:"auto"}} onClick={function(){setViewingPlayer(null);setViewingPlayerData(null);}}>
              ...
              {viewingPlayerData && (
                <div>
                  <div style={{padding:"20px 20px 16px",borderBottom:"1px solid "+C.border,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                    <div><div style={{fontSize:20,fontWeight:800}}>{viewingPlayerData.display_name}</div><div style={{fontSize:13,color:C.dim}}>@{viewingPlayerData.username} • {viewingPlayerData.role}</div></div>
                    <button onClick={function(){setViewingPlayer(null);setViewingPlayerData(null);}} style={{background:"none",border:"none",color:C.dim,cursor:"pointer",fontSize:22}}>✕</button>
                  </div>
                  <div style={{padding:"16px 20px"}}>
                    <div style={{display:"flex",justifyContent:"space-around",marginBottom:16,padding:"12px 0",background:C.card,borderRadius:10}}>
                      <div style={{textAlign:"center"}}><div style={{fontSize:24,fontWeight:800,color:C.green}}>{viewingPlayerData.stats?.win_rate||0}%</div><div style={{fontSize:10,color:C.dim}}>Win Rate</div></div>
                      <div style={{textAlign:"center"}}><div style={{fontSize:24,fontWeight:800,color:parseFloat(viewingPlayerData.stats?.roi||0)>=0?C.green:C.red}}>{viewingPlayerData.stats?.roi||0}%</div><div style={{fontSize:10,color:C.dim}}>ROI</div></div>
                      <div style={{textAlign:"center"}}><div style={{fontSize:24,fontWeight:800}}>{parseInt(viewingPlayerData.stats?.total_bets||0)}</div><div style={{fontSize:10,color:C.dim}}>Bets</div></div>
                    </div>
                    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr 1fr",gap:8,marginBottom:16}}>
                      <div style={{textAlign:"center",padding:8,background:C.card,borderRadius:8}}><div style={{fontSize:16,fontWeight:700,color:C.green}}>{parseInt(viewingPlayerData.stats?.wins||0)}</div><div style={{fontSize:10,color:C.dim}}>W</div></div>
                      <div style={{textAlign:"center",padding:8,background:C.card,borderRadius:8}}><div style={{fontSize:16,fontWeight:700,color:C.red}}>{parseInt(viewingPlayerData.stats?.losses||0)}</div><div style={{fontSize:10,color:C.dim}}>L</div></div>
                      <div style={{textAlign:"center",padding:8,background:C.card,borderRadius:8}}><div style={{fontSize:16,fontWeight:700,color:"#a3a3a3"}}>{parseInt(viewingPlayerData.stats?.pushes||0)}</div><div style={{fontSize:10,color:C.dim}}>P</div></div>
                      <div style={{textAlign:"center",padding:8,background:C.card,borderRadius:8}}><div style={{fontSize:16,fontWeight:700,color:parseInt(viewingPlayerData.stats?.net_profit||0)>=0?C.green:C.red}}>{parseInt(viewingPlayerData.stats?.net_profit||0)>=0?"+":""}{parseInt(viewingPlayerData.stats?.net_profit||0)}</div><div style={{fontSize:10,color:C.dim}}>Net</div></div>
                    </div>
                    ...
                    {viewingPlayerData.byType?.length>0 && (
                      <div style={{marginBottom:16}}>
                        <div style={{fontWeight:700,fontSize:14,marginBottom:8}}>By Bet Type</div>
                        {viewingPlayerData.byType.map(function(bt,i){
                          var wr=parseInt(bt.total)>0?Math.round((parseInt(bt.wins)/(parseInt(bt.wins)+parseInt(bt.losses)))*100):0;
                          return (
                            <div key={i} style={{display:"flex",justifyContent:"space-between",padding:"6px 0",borderBottom:i<viewingPlayerData.byType.length-1?"1px solid "+C.border:"none"}}>
                              <span style={{fontWeight:600,textTransform:"capitalize"}}>{bt.bet_type}</span>
                              <div style={{display:"flex",gap:12,fontSize:13}}><span style={{color:C.green}}>{bt.wins}W</span><span style={{color:C.red}}>{bt.losses}L</span><span style={{color:C.dim}}>{wr}%</span></div>
                            </div>
                          );
                    ...
                    {viewingPlayerData.recentBets?.length>0 && (
                      <div>
                        <div style={{fontWeight:700,fontSize:14,marginBottom:8}}>Recent Bets</div>
                        {viewingPlayerData.recentBets.map(function(b,i){
                          var col=b.result==="win"?C.green:b.result==="loss"?C.red:"#a3a3a3";
                          return (
                            <div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 0",borderBottom:i<viewingPlayerData.recentBets.length-1?"1px solid "+C.border:"none"}}>
                              <div>
                                <div style={{fontSize:13}}><span style={{color:C.dim}}>{tm(b.away_team).abbr}@{tm(b.home_team).abbr}: </span><strong>{betDesc(b.bet_type,b.pick,b.line,b.odds)}</strong></div>
                                <div style={{fontSize:11,color:C.dim}}>{new Date(b.created_at).toLocaleDateString("en-US",{month:"short",day:"numeric"})}</div>
                              </div>
                              <div style={{textAlign:"right"}}>
                                <div style={{fontWeight:700,color:col}}>{b.result==="win"?"+"+(b.payout-b.wager):b.result==="loss"?"-"+b.wager:"Push"}</div>
                                <div style={{fontSize:11,color:C.dim}}>{b.wager} wagered</div>
                              </div>
                            </div>
                          );
                        })}
        ...
        {tab==="admin" && myRole==="admin" && (
          <div>
            <Card style={{borderColor:C.purple+"44"}}>
              ...
              <div style={{display:"flex",gap:0,marginBottom:16,marginTop:12,borderRadius:8,overflow:"hidden",border:"1px solid "+C.border}}>
                {[["pending","Pending ("+members.filter(function(m){return m.status==="pending";}).length+")"],["active","Active"],["deactivated","Off"]].map(function(item){
                  return <button key={item[0]} onClick={function(){setAdminTab(item[0]);}} style={{flex:1,padding:"8px 0",background:adminTab===item[0]?C.purple:"transparent",color:"#fff",border:"none",cursor:"pointer",fontWeight:600,fontSize:12}}>{item[1]}</button>;
                })}
              </div>

              {adminTab==="pending" && (
                members.filter(function(m){return m.status==="pending";}).length===0 ?
                <div style={{textAlign:"center",padding:20,color:C.muted}}>None</div> :
                members.filter(function(m){return m.status==="pending";}).map(function(m){
                  return (
                    <div key={m.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"12px 14px",borderRadius:10,marginBottom:6,background:C.cardB}}>
                      <div><div style={{fontWeight:700}}>{m.display_name}</div><div style={{fontSize:12,color:C.dim}}>@{m.username}</div></div>
                      <div style={{display:"flex",gap:6}}><Btn small bg={C.green} onClick={function(){updateMember(m.id,"approve");}}>Approve</Btn><Btn small bg={C.red} onClick={function(){updateMember(m.id,"deactivate");}}>Deny</Btn></div>
                    </div>
                  );
                })
              )}

              {adminTab==="active" && members.filter(function(m){return m.status==="active";}).map(function(m){
                return (
                  <div key={m.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"12px 14px",borderRadius:10,marginBottom:6,background:C.cardB}}>
                    <div>
                      <div style={{fontWeight:700,display:"flex",alignItems:"center",gap:6}}>{m.display_name}{m.role==="admin" && <Badge text="Admin" color={C.purple}/>}</div>
                      <div style={{fontSize:12,color:C.dim}}>@{m.username} • {m.balance?.toLocaleString()} pts</div>
                    </div>
                    <div style={{display:"flex",gap:6}}>
                      {m.role!=="admin" && <Btn small bg={C.red} onClick={function(){updateMember(m.id,"deactivate");}}>Off</Btn>}
                      <Btn small bg={C.amber} onClick={function(){setAdjustModal(m);}}>Adjust</Btn>
                    </div>
                  </div>
                );
              })}
              ...
              {adminTab==="deactivated" && (
                members.filter(function(m){return m.status==="deactivated";}).length===0 ?
                <div style={{textAlign:"center",padding:20,color:C.muted}}>None</div> :
                members.filter(function(m){return m.status==="deactivated";}).map(function(m){
                  return (
                    <div key={m.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"12px 14px",borderRadius:10,marginBottom:6,background:C.cardB,opacity:0.7}}>
                      <div><div style={{fontWeight:700}}>{m.display_name}</div></div>
                      <Btn small bg={C.green} onClick={function(){updateMember(m.id,"reactivate");}}>Reactivate</Btn>
                    </div>
                  );
                })
              )}
            ...
            {adjustModal && (
              <Card style={{border:"1px solid "+C.amber}}>
                <div style={{fontWeight:700,marginBottom:8}}>Adjust: {adjustModal.display_name}</div>
                <div style={{fontSize:13,color:C.dim,marginBottom:10}}>Current: {adjustModal.balance} pts</div>
                <Input type="number" placeholder="Amount (+100 or -50)" value={adjustAmt} onChange={function(e){setAdjustAmt(e.target.value);}} style={{marginBottom:8}}/>
                <Input placeholder="Reason (optional)" value={adjustReason} onChange={function(e){setAdjustReason(e.target.value);}} style={{marginBottom:10}}/>
                <div style={{display:"flex",gap:8}}>
                  <Btn bg={C.amber} onClick={adjustBalance}>Apply</Btn>
                  <Btn bg="#374151" onClick={function(){setAdjustModal(null);setAdjustAmt("");setAdjustReason("");}}>Cancel</Btn>
                </div>
              </Card>
            )}

            <Card style={{borderColor:C.amber+"44"}}>
              <div style={{fontWeight:700,fontSize:15,marginBottom:4,color:C.amber}}>Scores</div>
              <div style={{fontSize:12,color:C.dim,marginBottom:12}}>Manual refresh from Odds API</div>
              <div style={{display:"flex",gap:8}}>
                <Btn small bg={C.amber} onClick={async function(){try{await api("/api/admin/refresh-scores",{method:"POST"});showMsg("Scores refreshed!");await refreshPool();}catch(e){showMsg(e.message,"error");}}}>Refresh Scores</Btn>
                <Btn small bg={C.blue} onClick={async function(){try{var d=await api("/api/admin/refresh-odds",{method:"POST"});showMsg("Odds: "+d.gamesUpdated+" games");setGames(await api("/api/games/upcoming"));}catch(e){showMsg(e.message,"error");}}}>Refresh Odds</Btn>
              </div>
            </Card>

            <Card>
              <div style={{fontWeight:700,fontSize:15,marginBottom:10}}>Settings</div>
              <div style={{display:"flex",justifyContent:"space-between",padding:"8px 0",borderBottom:"1px solid "+C.border}}><span style={{fontSize:13}}>Join Code</span><span style={{fontWeight:700,letterSpacing:2}}>{activePool?.join_code}</span></div>
              <div style={{display:"flex",justifyContent:"space-between",padding:"8px 0"}}><span style={{fontSize:13}}>Starting Balance</span><span style={{fontWeight:600}}>{activePool?.starting_balance||1000} pts</span></div>
            </Card>
          </div>
        )}
      </div>
